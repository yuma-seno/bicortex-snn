...
- [ ] **開発環境の準備 (Dev Containers)**
    ...
    - [ ] **コンテナの起動と動作確認**:
        - [ ] VS Codeで「Reopen in Container」を実行、使用しているGPUが Nvidia の場合は `python-nvidia`を選択し、動作を確認
            - [ ] Python実行確認 (requirements.txt が自動インストールされているか)
            - [ ] GPU認識確認: `python -c "import torch; print(torch.cuda.is_available())"` が `True` になるか
        - [ ] VS Codeで「Reopen in Container」を実行、使用しているGPUが AMD の場合は `python-amd`を選択し、同様に動作を確認
            - [ ] Python実行確認 (requirements.txt が自動インストールされているか)
            - [ ] GPU認識確認: `python -c "import torch; print(torch.cuda.is_available())"` が `True` になるか
- [x] **ドキュメント整備 [DOC]**
