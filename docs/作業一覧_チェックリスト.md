...
- [x] **固定結合 ($W_{fixed}$) - 事前学習/調整**: 
    - `Sensory` $\to$ `Concept`: 特徴抽出
    - `Concept` $\to$ `Motor`: 事前学習済み行動 (Reflex)
    - `Concept` $\leftrightarrow$ `Memory`: **Interface (Injection / Recall)** ※双方向とも固定
- [x] **可塑的結合 ($W_{plastic}$) - 学習対象**: 
    - `Memory` $\to$ `Memory`: **リカレント結合のみを学習** (SRG対象)
- [x] **E/Iバランス**: Dale's Lawに基づくリザーバ初期化 (興奮性80%:抑制性20%)
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/architecture.md`: 領域マップ、結合トポロジーの定義
...
- [x] **思考野の構築**:
    - [x] トレーニング後の理想的な思考野ニューロン結合パターンの設計とハードコーディング
