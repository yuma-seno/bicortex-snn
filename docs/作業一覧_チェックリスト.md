...
- [x] **クラス設計**: 領域定義 (`Sensory`/`Concept`/`Motor`/`Memory`) を含む `BiCortexEngine` の定義
- [x] **LIFダイナミクス**: 行列演算による膜電位更新ロジックの実装
- [x] **2つのトレース変数**:
    - [x] `x_fast` (即時トレース: ~5ms) の実装
    - [x] `e_trace` (適格性トレース: ~2.0s) の実装と減衰率調整
...
### 1.2 結合トポロジー構築
- [x] **領域マップ定義**: 
    - Thinking: Sensory(感覚) / Concept(概念) / Motor(運動)
    - Memory: Memory(記憶) のID割当
- [x] **固定結合 ($W_{fixed}$) - 事前学習**: 
    - `Sensory` $\to$ `Concept`: 特徴抽出
    - `Concept` $\to$ `Motor`: 事前学習済み行動 (Pre-trained Behavior)
    - `Concept` $\to$ `Memory`: Context Injection (ブロードキャスト)
- [x] **可塑的結合 ($W_{plastic}$) - 学習対象**: 
    - `Memory` $\to$ `Motor`: Interface結合 (SRG対象)
    - `Memory` $\to$ `Memory`: リカレント結合 (SRG対象)
- [x] **E/Iバランス**: Dale's Lawに基づくリザーバ初期化 (興奮性80%:抑制性20%)
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/architecture.md`: 3層思考野構造、結合ルールの詳細記述

### 1.3 学習アルゴリズム (SRG) 実装
- [x] **ゲート信号 $G(t)$**: `Concept` ニューロンの活動量に基づく動的判定ロジック
- [x] **更新則**: Gated Hebbian 則 ($\Delta W = \eta \cdot G \cdot Post \cdot Pre_{trace}$) の実装
- [x] **安定化処理**: Global Weight Decay (全体減衰) および重みクリッピングの実装
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/math_model.md`: SRG (Concept-Triggered) の定義修正

### 1.4 原理の動作検証 (パブロフ・テスト)
- [ ] **シナリオ作成**: ベル(予兆) → 空白時間 → エサ(報酬) の入力シーケンス作成
- [ ] **思考野の構築**:
    - [ ] トレーニング後の理想的な思考野ニューロン結合パターンの設計とハードコーディング
- [ ] **シミュレーション実行**: 学習前後での挙動比較
    - [ ] 初期ショート（漏洩）の特定と切断
    - [ ] 暴走（Bursting）の抑制と、順応（Adaptation）による自然な減衰の実現
- [ ] **可視化**:
    - [ ] アクションニューロンの活動履歴をCLIでプロット表示 (10ステップごとの発火数を横並びで表示)
    - [ ] アクションニューロンの活動履歴グラフ作成 (`reports/figures/pavlov_result.png`)
- [ ] **ドキュメント更新 [DOC]**
    - [ ] `docs/architecture.md`: リザーバ固定化と順応メカニズムの反映
    - [ ] `docs/math_model.md`: 適応的閾値の数式追加

### 1.5 判断能力の動作検証（2択パブロフ・テスト）
- [ ] **シナリオ作成**: 赤画像→報酬、青画像→報酬なし の識別学習
- [ ] **思考野の構築**:
    - [ ] トレーニング後の理想的な思考野ニューロン結合パターンの設計とハードコーディング
- [ ] **シミュレーション実行**: 学習前後での行動変化の確認
- [ ] **可視化**:
    - [ ] アクションニューロンの活動履歴をCLIでプロット表示 (10ステップごとの発火数を横並びで表示)
    - [ ] アクションニューロンの活動履歴グラフ作成 (`reports/figures/choice_pavlov_result.png`)
- [ ] **ドキュメント更新 [DOC]**
    - [ ] `docs/architecture.md`: 2択タスク対応の記述追加

## Phase 2: 視覚エンコーディング統合
### 2.1 視覚野バックボーン準備
- [x] **モデル選定**: `MobileNetV3-Small` (Pre-trained) のロード
- [x] **特徴抽出**: 最終層直前のFeature Mapを取り出すフックの実装
- [x] **エンコーディング**: 特徴量の値を電流値に変換するマッパーの実装
- [ ] **最適化検討**: ニューロン数増加に伴う、疎行列 (Sparse Matrix) 導入の検討と実装
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/encoding_spec.md` 作成: CNNの特徴マップ次元数と、それをどうSNN入力電流に変換したか（正規化手法など）を記述
