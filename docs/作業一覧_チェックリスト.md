...
- [x] **不応期 (Refractory Period)**: 発火後の強制リセット処理の実装
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/math_model.md` 作成: 実装したLIFモデルとトレース更新の数式、コード上の変数名の対応表を記述

### 1.2 結合トポロジー構築
- [x] **領域マップ定義**: `Thinking` / `Memory` のID範囲設定
- [x] **固定結合 ($W_{fixed}$)**: 思考野内部、および概念→記憶野の投射結合の初期化
- [x] **可塑的結合 ($W_{plastic}$)**: 記憶野リカレント、インターフェース結合の初期化 (Zero or Random initialization)
- [x] **抑制性ニューロン**: Dale's Lawに基づく抑制性結合 (20%) のマスク処理
- [x] **ドキュメント更新 [DOC]**
    - [x] `docs/architecture.md` 作成: ニューロンIDの割り当て範囲(Map)と、各結合行列の初期化戦略(スパース率など)を記録

### 1.3 学習アルゴリズム (SRG) 実装
- [x] **ゲート信号 $G(t)$**: 概念ニューロンの活動総和による閾値判定ロジック
- [x] **更新則**: $\Delta W = \eta \cdot G(t) \cdot Post \cdot Pre_{slow}$ の計算実装
- [x] **正規化/クリッピング**: 重みが発散しないための上限設定 (`np.clip`)
